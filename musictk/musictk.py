...
def play(curselect,player,loader=False):
    global started, once
    ...
    try:
        ...
        im       = Image.open(BytesIO(art_data)).resize((500,499),Image.ANTIALIAS).convert('RGB')
        
        player.config(track=title,album=album,art=im,artist=artist,backup_track=song[0])
    
    except IndexError:
        player.config(track=title,album=album,artist=artist,art=[],backup_track=song[0])
...
def directory(dir=''):
    ...
    if dir:
        path = filedialog.askdirectory(title='Choose a directory with songs',initialdir=dir)
    else:
        path = filedialog.askdirectory(title='Choose a directory with songs')

    load_songs(path,'dir') if path else ''

def load_songs(song_paths,path_type):
...
def file():
    ...
    path = filedialog.askopenfilenames(title='Choose song files',filetypes=(('MP3 file','*.mp3'),))
    load_songs(path)    
...
def next_prev_song(action,player):
...
def about():
    ...
    btn_cod = ttk.Button(frame, text='Source Code!', command=lambda: webbrowser.open(url_2, new=1))
...
def show_mini():
    ...
    def show_main(btn):
        ...
        file_menu.entryconfig(2,label='Switch to mini player',command=show_mini)

        ismini = False
    
    if song_list.size() == 0 and not once: 
        messagebox.showerror('Choose song','Play a song from main player first and then switch to mini-player')
        return
    ...
    file_menu.entryconfig(2,label='Switch to main player',command=lambda: show_main(btn))
...
def save_settings():
    ...
    try: idx = song_list.curselection()[0]
    except IndexError: idx = 0
...
song_list_label.grid(row=2,column=0,sticky='w',columnspan=2,pady=(15,5)) #pady=15,5
...
ttk.Button(left_frame, text='Add a song directory',command=directory).grid(row=4, column=0,ipady=5,pady=5)
ttk.Button(left_frame, text='Add a song file', command=file).grid(row=4, column=1,ipady=5,pady=5)
ttk.Button(left_frame, text='Empty the list', command=empty).grid(row=4, column=2,ipady=5,pady=5)

song_list.bind('<Double-Button-1>', lambda e: play(curselect=song_list.curselection()[0],player=init_player))
...
init_player = Player(right_frame,artist='-',album='-',track='-',
                         playfunc=lambda: play_song(),previousfunc=lambda :next_prev_song('prev',init_player)
                        ,nextfunc=lambda: next_prev_song('next',init_player),stopfunc=lambda: rewind(),theme='light')
...
try:
    root.bind('<Return>',lambda e: play(song_list.curselection()[0],player=init_player))
except IndexError:
    pass
...
file_menu.add_command(label='Switch theme', command=lambda: change_theme(init_player,file_menu))
